...
//保存bytes_per_block,blocks_per_key,blocks,size到kv的meta域中
int BitmapFreelistManager::create(uint64_t new_size, uint64_t granularity,
				  KeyValueDB::Transaction txn)
{
  bytes_per_block = granularity;//就是min_alloc_size,默认64k
...
void BitmapFreelistManager::_init_misc()//初始化掩码
{
  ...
  KeyValueDB::Transaction txn)
{
  ...
  uint64_t first_key = offset & key_mask;//按key对齐
  uint64_t last_key = (offset + length - 1) & key_mask;
  dout(20) << __func__ << " first_key 0x" << std::hex << first_key
	   << " last_key 0x" << last_key << std::dec << dendl;

  if (first_key == last_key) {//同一个key表示操作在一个段里面
    bufferptr p(blocks_per_key >> 3);//因为1个bit就可以代表一个block，所以除以8
    p.zero();//所有bit位置为0
    unsigned s = (offset & ~key_mask) / bytes_per_block;//key内的起始编号
    unsigned e = ((offset + length - 1) & ~key_mask) / bytes_per_block;//key内的结束编号
    for (unsigned i = s; i <= e; ++i) {//把从s到e的bit位置为1
      p[i >> 3] ^= 1ull << (i & 7);//i>>3定位该block在p里第几个字节，
    }
    string k;
    make_offset_key(first_key, &k);
    bufferlist bl;
    ...
    *_dout << dendl;
	...
	p[i >> 3] ^= 1ull << (i & 7);
      ...
      *_dout << dendl;
      txn->merge(bitmap_prefix, k, bl);
      first_key += bytes_per_key;//offset跳到下一个key
    }
    // middle keys
    while (first_key < last_key) {
      string k;
      ...
      dout(30) << __func__ << " 0x" << std::hex << first_key << std::dec
      	 << ": ";
      all_set_bl.hexdump(*_dout, false);//中间段设置为全1
      *_dout << dendl;
