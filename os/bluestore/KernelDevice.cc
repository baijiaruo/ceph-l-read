...
int KernelDevice::open(const string& p)
{
  path = p;
  int r = 0;
  dout(1) << __func__ << " path " << path << dendl;///var/lib/ceph/osd/ceph-0/block

  fd_direct = ::open(path.c_str(), O_RDWR | O_DIRECT);//无缓存打开
  if (fd_direct < 0) {
    r = -errno;
    derr << __func__ << " open got: " << cpp_strerror(r) << dendl;
    return r;
  }
  fd_buffered = ::open(path.c_str(), O_RDWR);//有缓存打开
  if (fd_buffered < 0) {
    r = -errno;
    derr << __func__ << " open got: " << cpp_strerror(r) << dendl;
    goto out_direct;
  ...
  r = posix_fadvise(fd_buffered, 0, 0, POSIX_FADV_RANDOM);//随机访问优化？
  if (r) {
    r = -r;
    derr << __func__ << " open got: " << cpp_strerror(r) << dendl;
    goto out_fail;
  ...
  r = _aio_start();//初始化libaio，以及创建一个_aio_thread线程
  if (r < 0) {
    goto out_fail;
  }

  fs = FS::create_by_fd(fd_direct);//创建底层FS实例
  assert(fs);

  // round size down to an even block
  size &= ~(block_size - 1);//size按照4k对齐
...
int KernelDevice::_aio_start()
{
  if (aio) {
    ...
    int r = aio_queue.init();//调用io_setup，初始化libaio的ctx，
    if (r < 0) {
      if (r == -EAGAIN) {
