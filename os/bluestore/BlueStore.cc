...
int BlueStore::_open_bdev(bool create)
{
  assert(bdev == NULL);
  string p = path + "/block";
  bdev = BlockDevice::create(cct, p, aio_cb, static_cast<void*>(this));//返回一个KernelDevice实例
  int r = bdev->open(p);//初始化libaio，FS，size等
  if (r < 0)
    goto fail;

  if (bdev->supported_bdev_label()) {//更新superblock
...
int BlueStore::_open_fm(bool create)
{
  ...
  if (create) {
    ...
    uint64_t reserved = ROUND_UP_TO(MAX(SUPER_RESERVED, min_alloc_size),
				    min_alloc_size);//superblock预留一个block
    ...
    if (cct->_conf->bluestore_debug_prefill > 0) {//默认是0，不会进
    ...
    db->submit_transaction_sync(t);//提交事务落db
  }
...
int BlueStore::_open_db(bool create)
{
  ...
  bool do_bluefs;
  if (create) {
    do_bluefs = cct->_conf->bluestore_bluefs;//默认true
  } else {
    string s;
  ...
  if (do_bluefs) {
    ...
    if (::stat(bfn.c_str(), &st) == 0) {//db有独立分区的场景
      ...
      if (bluefs->bdev_support_label(BlueFS::BDEV_DB)) {//如果是KernelDevice,则为true
    ...
    } else if (::lstat(bfn.c_str(), &st) == -1) {//没有指定db的分区，采用共享的方式
    ...
    r = bluefs->add_block_device(bluefs_shared_bdev, bfn);//添加一个BlueFS::BDEV_DB的KernelDevice
    if (r < 0) {
      derr << __func__ << " add block device(" << bfn << ") returned: " 
	   << cpp_strerror(r) << dendl;
      goto free_bluefs;
	...
	bdev->get_size() * (cct->_conf->bluestore_bluefs_min_ratio +
			    cct->_conf->bluestore_bluefs_gift_ratio);//0.04的bdev size
      initial = MAX(initial, cct->_conf->bluestore_bluefs_min);//至少1G
    ...
    if (::stat(bfn.c_str(), &st) == 0) {//有独立的wal分区的场景
	...
	bluefs->add_block_extent(
      ...
      bluefs_single_shared_device = false;
    } else if (::lstat(bfn.c_str(), &st) == -1) {//wal不是独立分区
...
int BlueStore::mkfs()
{
  dout(1) << __func__ << " path " << path << dendl;//path为本地目录，即/var/lib/ceph/osd/ceph-0/
  int r;
  uuid_d old_fsid;

  {
    string done;
    r = read_meta("mkfs_done", &done);//read_meta会先从block中前4k的superblock来读，如果读不到再读path下的文件，读mkfs_done文件，第一次返回不存在
    if (r == 0) {
  ...
  {
    string type;
    r = read_meta("type", &type);//读type文件，第一次返回不存在
    if (r == 0) {
      if (type != "bluestore") {
	...
	return -EIO;
      ...
      r = write_meta("type", "bluestore");//type写为bluestore
      if (r < 0)
        return r;
    }
  }
  ...
  r = _open_path();//保存目录path的fd到path_fd
  if (r < 0)
    return r;

  r = _open_fsid(true);//path目录下打开或创建一个fsid的文件
  if (r < 0)
    goto out_path_fd;

  r = _lock_fsid();//lock fsid文件
  if (r < 0)
    goto out_close_fsid;

  r = _read_fsid(&old_fsid);
  if (r < 0 || old_fsid.is_zero()) {//生成一个fsid
    if (fsid.is_zero()) {
  ...
  r = _setup_block_symlink_or_file("block", cct->_conf->bluestore_block_path,
				   ...
				   cct->_conf->bluestore_block_create);//创建符号链接文件
  if (r < 0)
    goto out_close_fsid;
  if (cct->_conf->bluestore_bluefs) {
  ...
  r = _open_db(true);//初始化bluefs，kvdb
  if (r < 0)
    goto out_close_bdev;
...
int BlueStore::_mount(bool kv_only)
{
  ...
  if (cct->_conf->bluestore_fsck_on_mount) {//默认false
