	      ...
	      uuid_d fsid, int whoami)
{
  ...
  store->set_cache_shards(1);  // doesn't matter for mkfs! 创建TwoQCache实例
  ...
  ThreadPool::TPHandle &handle)
{
   ...
   */
  float recovery_sleep = get_osd_recovery_sleep();//获取一次recovery需要sleep的时间
  {
    Mutex::Locker l(service.recovery_sleep_lock);
    if (recovery_sleep > 0 && service.recovery_needs_sleep) {//需要sleep的场景
      PGRef pgref(pg);
	...
	Mutex::Locker l(service.recovery_sleep_lock);
        ...
        service.queue_recovery_after_sleep(pgref.get(), queued, reserved_pushes);//重新入队
      });
...
void OSD::ShardedOpWQ::_process(uint32_t thread_index, heartbeat_handle_d *hb)
{
  ...
  *_dout << dendl;
  ...
  ThreadPool::TPHandle tp_handle(osd->cct, hb, timeout_interval,
				 suicide_interval);
  qi->run(osd, pg, tp_handle);//调用对应的函数，recovery场景则调用do_recovery
