  ...
  pg_shard_t peer, const PushReplyOp &op, PushOp *reply)
{
  ...
  if (pushing.count(soid) == 0) {
    ...
    PushInfo *pi = &pushing[soid][peer];
    bool error = pushing[soid].begin()->second.recovery_progress.error;//error默认是false

    if (!pi->recovery_progress.data_complete && !error) {//上一次push op并没有完成这个object的push场景
	...
	&(pi->stat));//创建一个新的pushop保存在reply里面
	...
	goto done;
      ...
      pi->recovery_progress = new_progress;
      return true;
    } else {//已经完成该object的push
      // done!
done:
      if (!error)
	get_parent()->on_peer_recover( peer, soid, pi->recovery_info);//从peer_missing上删掉

      get_parent()->release_locks(pi->lock_manager);
      object_stat_sum_t stat = pi->stat;
      eversion_t v = pi->recovery_info.version;
      pushing[soid].erase(peer);//从pushing里清除
      pi = NULL;

      if (pushing[soid].empty()) {
	if (!error)
	  get_parent()->on_global_recover(soid, stat, false);//一般的流程，触发恢复继续进行
	else
