  ...
  bool is_delete)
{
  ...
  if (degraded_object_entry != waiting_for_degraded_object.end()) {//等待的io请求重新入队列
  ...
  const hobject_t &soid)
{
  ...
  if (p != recovering.end()) {//该对象正在恢复中
    dout(7) << "object " << soid << " v " << v << ", already recovering." << dendl;
  } else if (missing_loc.is_unfound(soid)) {//在主missing列表中找不到这个对象
...
{
  // all clean?
  if (needs_recovery()) {//是否需要recovery
