...
protected:
  ...
  int default_send_priority;//message的默认优先级
  /// set to true once the Messenger has started, and set to false on shutdown
  bool started;//功能开关，默认false
  uint32_t magic;//simple和async都为0
  int socket_priority;//所有socket包的优先级
...
public:
   ...
   */
  CephContext *cct;
  int crcflags;//标识要校验CRC的类型，包括MSG_CRC_DATA,MSG_CRC_HEADER
