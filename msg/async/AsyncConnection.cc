...
ssize_t AsyncConnection::read_until(unsigned len, char *p)
{
  ...
  if (async_msgr->cct->_conf->ms_inject_socket_failures && cs) {//一般不会特意注入失败
    if (rand() % async_msgr->cct->_conf->ms_inject_socket_failures == 0) {
      ldout(async_msgr->cct, 0) << __func__ << " injecting socket failure" << dendl;
      cs.shutdown();
    }
  ...
  do {
    ...
    switch (state) {
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_MESSAGE:
        {
          if (policy.throttler_messages) {//osd的throttler_messages为空
      ...
      case STATE_OPEN_MESSAGE_THROTTLE_BYTES:
        {
          ...
          if (cur_msg_size) {
            if (policy.throttler_bytes) {
              ...
              if (!policy.throttler_bytes->get_or_fail(cur_msg_size)) {//true表示被流控
                ldout(async_msgr->cct, 10) << __func__ << " wants " << cur_msg_size << " bytes from policy throttler "
                                           << policy.throttler_bytes->get_current() << "/"
                                            ...
                                            bufferlist &authorizer_reply)
{
  ...
  uint64_t feat_missing = policy.features_required & ~(uint64_t)connect.features;//校验features是否一致
  if (feat_missing) {
