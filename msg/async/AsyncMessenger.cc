  ...
  for (int i = 0; i < conf->ms_bind_retry_count; i++) {
    ...
    if (listen_addr.get_port()) {
      worker->center.submit_to(worker->center.get_id(), [this, &listen_addr, &opts, &r]() {
        r = worker->listen(listen_addr, opts, &listen_socket);//socket的create，bind，listen等动作
      }, false);
      if (r < 0) {
...
void Processor::accept()
{
  ...
  while (true) {
    ...
    int r = listen_socket.accept(&cli_socket, opts, &addr, w);//封装accept获得的client连接的fd到cli_socket中，并且对fd设置opts，addr为client的地址
    if (r == 0) {
    ...
    cluster_protocol(0), stopped(true)
{
  ...
  if (stack->support_local_listen_table())//psoix场景processor_num始终为1
...
void AsyncMessenger::ready()
{
  ...
  stack->ready();
  if (pending_bind) {//RDMA才走的流程，正常的pending_bind为false
